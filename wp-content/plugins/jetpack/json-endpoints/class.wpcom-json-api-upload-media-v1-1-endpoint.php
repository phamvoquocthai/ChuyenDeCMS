	...
	public function callback( $path = '', $blog_id = 0 ) {
		...
		if ( count( $other_media_files ) > 0 || count( $media_urls ) > 0 ) {
			if ( is_multisite() ) { // Do not check for available space in non multisites.
				add_filter( '_upload_prefilter', array( $this, 'check_upload_size' ), 9 ); // used for direct media uploads.
				add_filter( '_sideload_prefilter', array( $this, 'check_upload_size' ), 9 ); // used for uploading media via url.
			}

			if ( defined( 'IS_WPCOM' ) && IS_WPCOM ) {
				require_lib( 'tos-acceptance-tracking' );
				add_filter( '_upload_prefilter', '\\A8C\\TOS_Acceptance_Tracking\\handle_uploads_wpcomtos_blog' );
			}
